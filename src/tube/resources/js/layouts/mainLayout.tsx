...
export default function MainLayout({ title=null,children }: { title?: string | null, children: React.ReactNode }) {
  const { name,auth, flash } = usePage<{ name: string, auth: Auth, flash: { success: string | null, error: string | null } }>().props;
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        ...
        {localFlash && (
        <Alert className="m-1 w-2/3 bg-green-700" variant="success">
          <CheckCircle2Icon />
